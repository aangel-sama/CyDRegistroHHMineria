...
export async function registerVacation({
  ...
  // 1) Construye el array de días hábiles (solo lunes a jueves, sin feriados)
  const dias = eachDayOfInterval({ start: fechaInicio, end: fechaFin }).filter(
    (d) => d.getDay() >= 1 && d.getDay() <= 4 && !esFeriado(d)
  );
  ...
  const conflictos = (overlap || []).filter(
    (r) =>
      r.estado === "Enviado" || (r.estado === "Borrador" && (r.horas || 0) > 0)
  );

  if (conflictos.length) {
    // extrae fechas únicas
    const fechasUnicas = Array.from(new Set(conflictos.map((r) => r.fecha)));
  ...
  //    - O si cubre 4 días hábiles → Enviado
  ...
  for (const dia of dias) {
    ...
    const horasDia = dia.getDay() === 4 ? 8 : 12; // jueves 8h, resto 12h
