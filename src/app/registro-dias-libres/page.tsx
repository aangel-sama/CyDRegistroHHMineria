...
export default function RegistroDiasLibres() {
  ...
  const [motivo, setMotivo] = useState<"Vacaciones" | "Licencia médica">(
    "Vacaciones"
  );
  ...
  const [registrosDiasLibres, setRegistrosDiasLibres] = useState<
    PeriodoLibre[]
  >([]);
  ...
  const loadRegistrosDiasLibres = useCallback(async (email: string) => {
    const { data, error } = await supabase
      ...
      .eq("correo", email)
      .in("proyecto", ["GMI-Vacaciones", "GMI-Licencias"])
    ...
    const vacFechas = rows
      .filter((r) => r.proyecto === "GMI-Vacaciones" && r.horas > 0)
      .map((r) => r.fecha);
    const licFechas = rows
      .filter((r) => r.proyecto === "GMI-Licencias" && r.horas > 0)
      .map((r) => r.fecha);
    const vacPeriodos = agruparFechas(vacFechas).map((p) => ({
      ...p,
      motivo: "Vacaciones" as Motivo,
    }));
    const licPeriodos = agruparFechas(licFechas).map((p) => ({
      ...p,
      motivo: "Licencia médica" as Motivo,
    }));
              ...
              <div className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                    setMotivo(
                      e.target.value as "Vacaciones" | "Licencia médica"
                    )
                  }
            ...
            <section className="mt-8 w-full max-w-4xl bg-white rounded-xl shadow-md p-6">
              ...
              <ul className="list-disc pl-5 space-y-1 text-[#374151]">
                {registrosDiasLibres.map(({ start, end, motivo }) => (
                  ...
                  >
                    <span>
                      {format(parseISO(start), "dd/MM/yyyy")} –{" "}
                      {format(parseISO(end), "dd/MM/yyyy")} | {motivo}
                    </span>
                    <button
                      onClick={() => handleEliminar(start, end, motivo)}
